...
// creates a new array populated with the results of calling currentQuestion on every element in the calling array.
    const quizQuestions = logos.map(function(currentQuestion, index) {
        const question = `
            <div class="imgLogo">
...
// starting score should be zero
  ...
  // then listen for change event on radio button
    ...
    // if correct
    if (userChoice === logos[currentQuestionIndex].correctAnswer) {
  ...
  // listen for click on results button to restart entire game
